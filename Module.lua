...
-- // Create circle
local silentCircle = AimingSettings.SilentFOVSettings.Circle
silentCircle.Transparency = 1
silentCircle.Thickness = 2
silentCircle.Color = AimingSettings.SilentFOVSettings.Colour
...
-- // Update Silent FOV
function Aiming.UpdateSilentFOV()
	-- // Make sure the circle exists
	...
	-- // Vars
	local MousePosition = GetMouseLocation(UserInputService) + AimingSettings.Offset
	local Settings = AimingSettings.SilentFOVSettings

	-- // Set Circle Properties
	silentCircle.Position = MousePosition
	silentCircle.Color = Settings.Colour
	silentCircle.Visible = Settings.Enabled and Settings.Visible

	-- // Set radius based upon type
	if (Settings.Type == "Dynamic") then
		-- // Check if we have a target
		if (not Aiming.Checks.IsAvailable()) then
			silentCircle.Radius = (Settings.Scale * 3)
			return silentCircle
		end
		...
		-- // Calculate distance, set
		local Distance = (PartInstance.Position - TargetPart.Position).Magnitude
		silentCircle.Radius = math.round((Settings.DynamicFOVConstant / Distance) * 1000)
	else
		silentCircle.Radius = (Settings.Scale * 3)
	end
